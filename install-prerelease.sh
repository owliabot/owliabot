...
if [ -f "${SCRIPT_DIR}/install.sh" ]; then
  ...
  # When piped via curl, download install.sh and run it
  exec bash <(curl -fsSL https://raw.githubusercontent.com/owliabot/owliabot/develop/install.sh) --channel develop "$@"
fi
