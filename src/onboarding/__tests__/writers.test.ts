...
describe("writers step", () => {
  ...
  describe("buildDockerComposeYaml", () => {
    it("generates valid docker-compose structure", () => {
      const yaml = buildDockerComposeYaml("~/.owliabot", ["TZ=UTC"], "8787", "ghcr.io/owliabot/owliabot:latest");
    ...
    it("uses correct port mapping", () => {
      const yaml = buildDockerComposeYaml("~/.owliabot", ["TZ=UTC"], "9090", "img:latest");
      expect(yaml).toContain('"127.0.0.1:9090:8787"');
    });

    it("uses correct timezone", () => {
      const yaml = buildDockerComposeYaml("~/.owliabot", ["TZ=America/New_York"], "8787", "img:latest");
      expect(yaml).toContain("TZ=America/New_York");
    });

    it("mounts config and workspace volumes", () => {
      const yaml = buildDockerComposeYaml("~/.owliabot", ["TZ=UTC"], "8787", "img:latest");
