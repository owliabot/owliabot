  ...
  try {
    ...
    // Diagnostic: log LLM input summary
    {
      ...
      const inputTotalLen = chatMessages.reduce((sum, m) => {
        ...
        if (Array.isArray(m.content)) return sum + m.content.reduce((s: number, c: any) => s + (c.text?.length ?? 0), 0);
        return sum;
      }, 0);
    ...
    if (extracted.trim().length === 0) {
      ...
      const lastContentTypes = lastMessage?.content?.map((c: any) => c.type) ?? [];
      log.warn(
        `[LLM-final] empty text extraction — lastRole=${lastRole} contentTypes=[${lastContentTypes}] iterations=${iterations} toolCalls=${toolCallsCount}. Falling back to apology.`
      );
    }
    const finalContent =
...
function handleAgentEvent(
  ...
  switch (event.type) {
    ...
    case "turn_end": {
      ...
      if (event.message) {
        const outText = event.message.content
          .filter((c: any) => c.type === "text")
          .map((c: any) => c.text)
          .join("");
        const toolUseCount = event.message.content.filter((c: any) => c.type === "tool_use").length;
        const outSummary = outText.length > 50 ? outText.slice(0, 50) + "…" : outText;
        log.info(
          `[LLM-out] textLen=${outText.length} toolCalls=${toolUseCount} summary="${outSummary || "(no text, tool-only)"}"`
        );
      }
      break;
    }
  ...
  try {
    while (iteration < maxIterations) {
      ...
      // Diagnostic: log input
      {
        ...
        const lastUserText = typeof lastUser?.content === "string" ? lastUser.content : JSON.stringify(lastUser?.content ?? "").slice(0, 50);
