...
describe("oauth", () => {
  ...
  describe("getAllOAuthStatus", () => {
    it("should return status for openai-codex only (anthropic removed)", async () => {
      const mockCredentials = {
        access: "access_token",
        refresh: "refresh_token",
        expires: Date.now() + 3600000,
      ...
      // Only openai-codex should be present (anthropic OAuth removed)
      expect(statuses["openai-codex"].authenticated).toBe(true);
      expect(statuses).not.toHaveProperty("anthropic");
    });
  });
