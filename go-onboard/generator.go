...
func BuildAppConfig(a Answers) AppConfig {
	...
	cfg.System.Exec.CommandAllowList = []string{
...
func BuildDockerComposeYAML(configDir string, timezone string, gatewayPort string, image string) string {
...
services:
  owliabot:
    ...
    ports:
      - "127.0.0.1:%s:8787"
    volumes:
      - %s:/home/owliabot/.owliabot
      - %s/workspace:/app/workspace
    environment:
      - TZ=%s
    ...
    healthcheck:
      ...
      retries: 3
      start_period: 10s
`, image, gatewayPort, configDir, configDir, timezone)
}
